...
//@types/d3 is required to be installed to import d3 stuff as this doesnt have index.js
...
export class ComboChartComponent extends BaseChartComponent {
  ...
  trackBy(index: any, item: { name: any; }): string {
    return `${item.name}`;
  }
  ...
  getXScaleLine(domain: string | any[], width: number): any {
    let scale;
